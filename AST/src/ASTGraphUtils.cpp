...
namespace Decompiler {
    size_t find_merge_block(const Graph &cfg, size_t branch_true, size_t branch_false) {
        ...
        q_a.push(branch_true); q_b.push(branch_false);
        ...
        while (!q_a.empty() || !q_b.empty()) {
            if (!q_a.empty()) {
                const size_t curr_a = q_a.front(); q_a.pop();
                ...
                for (const auto succ : cfg.blocks[curr_a].successors) {
                    ...
                    if (visited_b.contains(succ)) return succ;
                    if (visited_a.insert(succ).second) q_a.push(succ);
                }
            }
            if (!q_b.empty()) {
                const size_t curr_b = q_b.front(); q_b.pop();
                ...
                for (const auto succ : cfg.blocks[curr_b].successors) {
                    ...
                    if (visited_a.contains(succ)) return succ;
                    if (visited_b.insert(succ).second) q_b.push(succ);
                }
            }
        }
