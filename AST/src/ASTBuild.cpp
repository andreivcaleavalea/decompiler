...
namespace Decompiler {
    namespace {
        std::string conditionOperandText(const std::vector<IRInstruction>& instrs, const size_t beforeIndex, const IROperand& operand, const StackFrameLayout layout) {
            if (operand.type == OpType::MEM && operand.memory.has_value()) {
                if (operand.memory->absolute.has_value()) {
                    if (const std::string globalSymbol = ASTDetail::globalSymbolForAddress(*operand.memory->absolute, operand.memory->sizeBytes); !globalSymbol.empty()) {
                        return globalSymbol;
                    }
                }
        ...
        void collectConditionTempDefinition(const std::vector<IRInstruction>& instrs, const size_t beforeIndex, const IROperand& operand, const StackFrameLayout layout, std::vector<size_t>& eraseIndices) {
            const std::string value = ASTDetail::normalizeOperandForDisplay(operand.value, layout);
            if (!ASTDetail::isTemporaryName(value)) {
                return;
            }
    ...
    public:
        ...
        bool try_create_while(size_t& curr, const bool is_loop_header, std::vector<IRInstruction> instrs) {
            ...
            if (!instrs.empty()) {
                ...
                seq->calling_convention = calling_convention_;
        ...
        while (curr != stop_block && curr != static_cast<size_t>(-1) && curr < cfg.blocks.size() && !visited.contains(curr)) {
            ...
            if (block.successors.size() == 2 && !instrs.empty()) {
                ...
                if (!condition.has_value()) {
                    auto seq = std::make_unique<SimpleBlockNode>();
                    seq->block_index = curr;
                    seq->calling_convention = calling_convention;
