...
namespace Decompiler {
    ...
    std::vector<std::string> SimpleBlockNode::print(int indent) const {
        ...
        const auto normalizeOperand = [&](const std::string& operand) {
        ...
        const auto isComposableTemporary = [&](const std::string& value) {
            return isTemporaryName(normalizeOperand(value));
        };
        const auto shouldFlushPendingCallArgumentsBefore = [&](const IRInstruction& instruction) {
            if (pendingCallArguments.empty() || isEpilogueNoise(instruction)) {
                return false;
            }

            if (!isPureExpressionInstruction(instruction)) {
                return true;
            }

            const std::string definedRegister = normalizedRegisterBase(IRProperties::operandAt(instruction, 0).value);
            if (definedRegister.empty()) {
                return false;
            }

            for (const auto& [_, argument] : pendingCallArguments) {
                if (normalizedRegisterBase(argument.register_name) == definedRegister) {
                    return true;
                }
            }
            return false;
        ...
        for (size_t i = 0; i < instructions.size(); ++i) {
            ...
            if (inst.type == IRType::CALL) {
                ...
                if (isTemporaryName(callName)) {
                    callName = normalizeImmediateForDisplay(substituteTempFromDefinitions(instructions, i, IRProperties::operandAt(inst, 0).value, stackFrameLayout));
                }
                ...
                if (spillIndex < instructions.size()) {
                    ...
                    const std::string spillSrc = normalizeOperand(IRProperties::operandAt(spill, 1).value);
            ...
            if (i + 2 < instructions.size()) {
                ...
                if (inst.type == IRType::LOAD
                    ...
                    && toStackSymbol(IRProperties::operandAt(inst, 1).value, loadedVar, stackFrameLayout)
                    && toStackSymbol(IRProperties::operandAt(storeInst, 0).value, storedVar, stackFrameLayout)
                    && loadedVar == storedVar
                    && IRProperties::operandAt(storeInst, 1).value == IRProperties::operandAt(addInst, 0).value) {
                    const std::string rhs = normalizeImmediateForDisplay(normalizeOperand(IRProperties::operandAt(addInst, 2).value));
            ...
            if (inst.type == IRType::LOAD) {
                op = "assign";
                if (toStackSymbol(IRProperties::operandAt(inst, 1).value, stackSymbol, stackFrameLayout)) {
                    src = stackSymbol;
                }
            } else if (inst.type == IRType::STORE && toStackSymbol(IRProperties::operandAt(inst, 0).value, stackSymbol, stackFrameLayout)) {
                op = "assign";
                dest = stackSymbol;
            ...
            if (shouldFlushPendingCallArgumentsBefore(inst)) {
                flushPendingCallArguments();
            }
                ...
                if (forwardedIndex < instructions.size()) {
                    ...
                    if ((forwarded.type == IRType::ASSIGN || forwarded.type == IRType::STORE) && IRProperties::operandAt(forwarded, 1).value == IRProperties::operandAt(inst, 0).value) {
                        ...
                        if (forwarded.type == IRType::STORE) {
                            std::string stackSymbol;
                            if (toStackSymbol(IRProperties::operandAt(forwarded, 0).value, stackSymbol, stackFrameLayout)) {
            ...
            if (rendersAssignment) {
                ...
                if (!src.empty()) {
                    result.push_back(spaces + dest + " = " + simplifyExpressionText(std::string(inst.type == IRType::NEG ? "-" : "~") + src));
                } else {
                    result.push_back(spaces + op + " " + dest);
                }
