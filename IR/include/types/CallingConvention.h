...
{
enum class CallingConvention { Unknown, SystemV, Win64 };

enum class StackFrameLayout { Generic, Win64HomeSlots };
...
inline std::optional<size_t> argumentIndexForRegisterBase(const std::string& base, const CallingConvention callingConvention)
{
    ...
    if (base == "rdi" || base == "rcx")
        return 0;
    if (base == "rsi" || base == "rdx")
        return 1;
    if (base == "r8")
        return 2;
    if (base == "r9")
...
inline bool isReturnRegisterName(const std::string& registerName)
{
    return normalizedRegisterBase(registerName) == "rax";
}

inline bool isStackPointerRegisterName(const std::string& registerName)
{
    return normalizedRegisterBase(registerName) == "rsp";
}

inline bool isFramePointerRegisterName(const std::string& registerName)
{
    return normalizedRegisterBase(registerName) == "rbp";
}

inline bool isStackOrFrameRegisterName(const std::string& registerName)
{
    return isStackPointerRegisterName(registerName) || isFramePointerRegisterName(registerName);
}
