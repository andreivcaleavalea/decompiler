...
namespace Decompiler::ASTDetail {
    ...
    std::string globalSymbolForAddress(const uint64_t address, const size_t sizeBytes) {
        for (const auto& region : g_globalMemoryRegions) {
            ...
            if (address >= region.start && address - region.start < region.size) {
                const std::string symbol = "global_" + sanitizeSymbolPart(region.sectionName) + "_" + hexAddress(address);
    ...
    std::string normalizeOperandForDisplay(const std::string& operand) {
        std::string symbol;
        if (toStackSymbol(operand, symbol)) {
            return symbol;
        }
        return operand;
    }
    ...
    std::string normalizedValueForDisplay(const std::string& operand) {
        return normalizeImmediateForDisplay(normalizeOperandForDisplay(operand));
    }
    ...
    namespace {
        ...
        std::string resolveOperandFromDefinitions(const std::vector<IRInstruction>& instrs,
            ...
            if (!isTemporaryName(operand) || cmpIndex == 0 || active.contains(operand)) {
                return normalizedValueForDisplay(operand);
            }
            ...
            for (size_t i = cmpIndex; i-- > 0;) {
                ...
                if (inst.type == IRType::ASSIGN || inst.type == IRType::SEXT || inst.type == IRType::LOAD || inst.type == IRType::LOAD_CONST) {
                    ...
                    if (isTemporaryName(sourceOperand.value) && sourceOperand.value != operand) {
                        const std::string value = resolveOperandFromDefinitions(instrs, i, sourceOperand.value, active);
                        active.erase(operand);
                        return value;
                    }
                    active.erase(operand);
                    return normalizeImmediateForDisplay(operandForDisplay(sourceOperand));
                }

                if (inst.type == IRType::ADD) {
                    const std::string lhs = resolveOperandFromDefinitions(instrs, i, normalizedValueForDisplay(IRProperties::operandAt(inst, 1).value), active);
                    const std::string rhs = resolveOperandFromDefinitions(instrs, i, normalizedValueForDisplay(IRProperties::operandAt(inst, 2).value), active);
                    active.erase(operand);

                    if (lhs == operand && rhs == normalizedValueForDisplay(IRProperties::operandAt(inst, 2).value)) {
                        return normalizedValueForDisplay(operand);
                    }
                    return lhs + " + " + rhs;
                }
                ...
                if (inst.type == IRType::NEG || inst.type == IRType::NOT) {
                    const std::string value = resolveOperandFromDefinitions(instrs, i, normalizedValueForDisplay(IRProperties::operandAt(inst, 1).value), active);
                    active.erase(operand);
                    return (inst.type == IRType::NEG ? "-" : "~") + value;
                }
                if (inst.type == IRType::LEA) {
                    active.erase(operand);
                    return normalizeOperandForDisplay(IRProperties::operandAt(inst, 1).value);
                }
