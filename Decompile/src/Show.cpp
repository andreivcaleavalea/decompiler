...
namespace Decompiler {
    namespace {
        ...
        bool isRuntimeReachabilityStop(const std::string& functionName) {
            return functionName == "__main" ||
                   functionName == "mainCRTStartup" ||
                   functionName == "WinMainCRTStartup" ||
                   functionName.starts_with("__do_global_") ||
        ...
        std::vector<std::string> decompileFunctionBody(
            ...
            const auto ast = build_ast(graph, dominators, postdominators, 0, static_cast<size_t>(-1), static_cast<size_t>(-1), static_cast<size_t>(-1), function.calling_convention);
        ...
        std::vector<DecompilerFunction> filterMainReachableFunctions(const std::vector<DecompilerFunction>& functions) {
            ...
            std::vector<DecompilerFunction> selected;
